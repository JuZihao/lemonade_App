...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        when (lemonadeState) {
            ...
            SQUEEZE -> {
                lemonSize -= 1
                squeezeCount += 1
                lemonadeState = if (lemonSize != 0) SQUEEZE else DRINK
            }
            DRINK -> {
    ...
    private fun setViewElements() {
        ...
        when (lemonadeState) {
            SELECT -> {
                text = getString(R.string.lemon_select)
                drawableResource = R.drawable.lemon_tree
            }
            SQUEEZE -> {
                text = getString(R.string.lemon_squeeze)
                drawableResource = R.drawable.lemon_squeeze
            }
            DRINK -> {
                text = getString(R.string.lemon_drink)
                drawableResource = R.drawable.lemon_drink
            }
            else -> {
                text = getString(R.string.lemon_empty_glass)
        ...
        lemonImage!!.contentDescription = drawableResource.toString()
    }
